 ...
 *
 * The followings are the available columns in table '{{call_type}}':
 * @property integer $id
...
class CallType extends UModel
{
	...
	public function rules()
	{
		...
		return array(
			array('string, name', 'required'),
			array('string, name', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, string, name', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return CallType the static model class
	 */
